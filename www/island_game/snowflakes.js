...
export function snowflakesInit() {
  ...
  const snowflakesPoints = new THREE.Points(
    g.SNOWFLAKES.GEOMETRY,
    snowflakeMaterial
  );
...
function snowflakesGetPositions() {
  ...
  return [
    ...
    (Math.random() * 0.2 + 0.9) * g.SNOWFLAKES.SPAWN_HEIGHT +
      g.CAMERA.position.y,
    (Math.random() - 0.5) *
      spread *
      g.SNOWFLAKES.SPAWN_RADIUS *
...
function updateDeadSnowflakes(i, deltaTime) {
...
export function snowflakesUpdate(deltaTime) {
  for (let i = 0; i < g.SNOWFLAKES.NUM; i++) {
    ...
    // if snowflake is dead
    if (g.SNOWFLAKES.GEOMETRY.attributes.lifetime[i] < 1) {
      updateDeadSnowflakes(i, deltaTime);
      continue;
    }
    ...
    // );
    const shape = new g.PHYSICS.RAPIER.Ball(0.5);
    // position of the snowflake
    const shapePos = {
      x: g.SNOWFLAKES.GEOMETRY.attributes.position.array[i * 3],
      y: g.SNOWFLAKES.GEOMETRY.attributes.position.array[i * 3 + 1],
      z: g.SNOWFLAKES.GEOMETRY.attributes.position.array[i * 3 + 2],
    };
    const shapeRot = { w: 1.0, x: 0.0, y: 0.0, z: 0.0 };

    let result = false;
    g.PHYSICS.WORLD.intersectionsWithShape(
      shapePos,
      shapeRot,
      shape,
      (handle) => {
        // console.log("The collider", handle, "intersects our shape.");
        result = true; // Return `false` instead if we want to stop searching for other colliders that contain this point.
      }
    );
